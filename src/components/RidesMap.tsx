...
export default function RidesMap({ rides }: RidesMapProps) {
  ...
  useEffect(() => {
    if (rides.length > 0) {
      const filteredRides = rides.filter((ride: RidesMapProps['rides'][0]) => {
      ...
      const newMarkers = filteredRides.map((ride: RidesMapProps['rides'][0]) => ({
        id: ride._id,
        position: {
  ...
  if (loadError) {
    return (
      <div className="w-full h-[300px] bg-red-50 rounded-xl flex items-center justify-center text-red-600">
